...
/*
트리거 (TRIGGER)
1. DML (INSERT, UPDATE, DELETE ) 작업 후 자동으로 실행되는 데이터 베이스객체이다
2. 행(ROW)단위로 트리거가 동작
3. 종류
...
-- OLD 테이블 
    ...
    2. :OLD 방식으로 호출 
    3. AFTER 트리거와 함께 사용 
        1) AFTER INSERT : NULL 값이 있다 
        2) AFTER UPDATE : UPDATE 이전 데이터(수정 전의 데이터)
...
-- 트리거 실습 문제 
-- 사원(EMPLOYEES) 테이블에서 삭제할 사원정보를 퇴사자 테이블로 이동 이름 : RETIRE_TRIG  
-- 1.사원 테이블의 구조를 복사하여 퇴사자 테이블을 생성한다 
-- 2. 퇴사자(RETIRES) 테이블에 퇴사번호(RETIRES_ID), 퇴사일자 (RETIRE_DATE) 칼럼을 추가한다
-- 3. 퇴사번호(RETIRE_ID) 칼럼을 기본키로 지정한다
-- 4. 퇴사번호를 생성할 RETIRE_SEQ 시퀀스를 생성한다 
--5. RETIRE_TRIG 트리거를 생성한다
        --1) AFTER DELETE 트리거 
        --2) OLD 테이블의 데이터를 모두 퇴사자 테이블 
        --3) 퇴사번호는 시퀀스, 퇴사일자 SYSDATE로 처리해서 삽입한다. (시퀀스, SYSDATE 이용)
...
CREATE OR REPLOACE PROCEDURE RETIRE_TRIG
IS
    RETIRES EMPLOYEES.RETIRES_ID%TYPE;
