...
/*
    ...
    2. 처리할 일 
         ...
         3) 고객 테이블의 고객 포인트를 수정(UPDATE)한다
...
CREATE TABLE PROD_TBL (
   /* 프로덕트 코드*/ P_CODE NUMBER NOT NULL, 
...
CREATE OR REPLACE PROCEDURE BUY_PROD(
        ...
        PCODE IN RPOD_TBL.P_CODE%TYPE, 
        BUY_AMOUNT IN BUY_TBL.B_AMOUNT%TYPE
)
IS -- IS 든 ,AS든 적어줘야 함 
BEGIN 
--1) 구매내역 테이블에 구매 내역을 추가(INSERT)한다 
    INSERT INTO BUY_TBL(B_NO,C_NO,P_CODE, B_AMOUNT) VALUES(BUY_SEQ.NEXTVAL, CNO, PCODE, BUY_AMOUNT); --구매 내역에 추가하는 쿼리 
-- 시퀀스에 대한 자세한 이해가 필요함 , 개념 잘 알고 적재적소에 쓸 것 
--2_ 구매내역 테이블에 구매 내역을 추가(INSERT)한다
    UPDATE PROD_TBL SET P_STOCK = P_STOCK - BUY_AMOUNT WHERE P_CODE = PCODE;
    
--3. 고객 테이블의 포인트를 수정한다 (UPDATE)
-- 총 구매액의 10%를 정수로 올림처리해서 포인트로 준다 
-- 가격 * 구매수량 * 0.1 을 올림처리 하는게 CEIL 인데 
UPDATE CUST_TBL SET C_POINT = C_POINT + CEIL((SELECT P_PRICE FROM PROD_TBL WHERE P_CODE = PCODE) * BUY_AMOUNT * 0.1) WHERE C_NO = CNO;-- = 이게 POINT임
-- 이걸 고객 포인트에 올릴 예정 : UPDATE CUST_TBL SET C_POINT = C_POINT

--4. 커밋해라
COMMIT;

--문제가 발생할 수도 있으니 EXCEPTION을 잡아줘라
EXCEPTION 
    WHEN OTHERS THEN 
     -- 모든 예외에 대해서 처리  1. 예외 사유 확인 메세지
...
-- BUY_PROC 프로시저 호출 
-- 1)EXECUTE BUY_PROC();
EXECUTE BUY_PROC(1 , 1000, 10); -- 고객번호 1, 제품코드 1000, 구매수량1-

/*2. BEGIN
    BUY_PROC(); -- EXECUTE 없이 소환 
END; 
*/ 
