...
-- 2. SELECT절에서 조회하려는 칼럼은 "반드시" GROUP BY 절에 있어야 한다.(필수규칙) 

--1. 동일한 DEPARTMENT_ID를 그룹화하여 조호하시오 
...
--4. GROUP BY절을 지정하면 가틍ㄴ 그룹끼리 집계함수가 적용됨
SELECT
        DEPARTMENT_ID
     ,  COUNT(*) AS 부서별사원수
  ...
  GROUP BY
        DEPARTMENT_ID;
--참고. GROUP BY 없이 집계함수 사용 
SELECT
        DISTINCT DEPARTMENT_ID
    ,   COUNT(*) OVER(PARTITION BY DEPARTMENT_ID) -- 디파트 먼트 아이디별로 모아서 할것 
...
--   해설 ) DEPARTMENT_ID가 NULL 부서의 제외는 GROUP BY 이전에 처리할 수 있으므로 WHERE절로 처리
SELECT 
        DEPARTMENT_ID
      , COUNT(*) AS 부서별사원수
    FROM
...
--  해설) 부서별 인원 수는 GROUP BY 이후에 확인할 수 있으므로 HAVING절에서 조건을 처리하시오 
