...
-- 날짜 형식을 변경하는 함수 이름은 무엇인가 TO_DATE(날짜를 만드는게 목적)
-- TO_CHAR : 날짜 형식을 변경하는 함수
SELECT TO_CHAR(BIRTHDATE, 'YYYY-MM-DD') AS "생 일"
FROM STUDENT;

-- ''이 안되면 "" 사용 "" 써도 됨 다만 공백이 있을 시 "",'' 사용 
...
30일 비밀번호 변경 시 응용해 사용 가능 
/*
3.
학생(STUDENT) 테이블의 키(HEIGHT) 칼럼의 데이터 중에서 가장 높은 키를 조회하는 쿼리문을 작성하시오. (5점)
*/

SELECT SNAME, MAX(HEIGHT) AS 가장 높은 키
FROM STUDENT;
MAX는 한개 값이기 때문에 안 돌아가는 것임
...
S.SNAME은 여러개이기 때문에 COUNT를 써야 하는것 
...
*/
RANK() OVER 함수가 연봉순으로 조회하는 것까지 하는 것 
