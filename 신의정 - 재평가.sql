...
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(B.BUY_AMOUNT) AS 총구매액
...
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, SUM(B.PROD_PRICE * B.BUY_AMOUNT) AS 총구매액
...
-- 작성
--OR REPACE가 있으면 제거하라는 뜻 
--전달하는 매개변수가 없기 때문에 괄호가 없어도 됨
--이름과 동일한 타입을 가지기 위해서 USER_NAME을 쓰기로
  --SELECT 칼럼 INTO 변수
...
-- 사용자 함수는 SELECT 확인 PROCEDURE 는 EXCUTE로 확인 하는 편
...
-- 팁 모든 고객의 등급을 보고 싶다면? SELECT USER_NAME AS 고객명, GET_GRADE(USER_ID) AS 등급 FROM USERS;
