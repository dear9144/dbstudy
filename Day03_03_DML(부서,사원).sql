...
ALTER TABLE DEPARTMENT_TBL
    ADD CONSTRAINT PK_DEPT PRIMARY KEY(DEFT_NO);
...
ALTER TABLE EMPLOYEE_TBL
    ADD CONSTRAINT FK_EMP_DEPT FOREIGN KEY(DEPART) 
        REFERENCES DEPARTMENT_TBL(DEFT_NO)
            ON DELETE SET NULL;
-- 시퀀스(번호 생성기)삭제하기
...
CREATE SEQUENCE DEPARTMENT_SEQ
    ...
    NOMAXVALUE --번호의 상한선이 없음 (생략 가능) MAXVALUE 100 : 번호를 100까지만 생성한다 
    NOMINVALUE --번호의 하한선이 없다.(생략 가능) MINVALUE 100 : 번호를 
    NOCYCLE -- 번호 순환이 없다 (생략 가능) CYCLE : 번호가 MAXVALUE에 도달하면 다음 번호는 MINVALUE이다.
    NOCACHE -- 메모리 캐시를 사용하지 않는다. CATCH : 메모리 캐시를 사용한다(사용하지 않는 것이 좋다) 시간이 오래 걸리니 미리 번호를 뽑아내겠다
    ORDER -- 번호 건너뛰기가 없다 NOORDER : 번호 건너뛰기가 가능하다  
;
--시퀀스만들기(데이터 입력 전에 넣어야 함)
--CREATE SEQUENCE DEPARTMENT_SEQ;
-- 시퀀스에서 번호 뽑는 함수 : NEXTVAL
-- SELECT DEPARTMENT_SEQ.NEXTVAL FROM DUAL;
-- 테이블에 없는 내용을 조회하는 방법 노션 (모든 DB가 그런건 X) 
 
--데이터 입력하기 
INSERT INTO DEPARTMENT_TBL(DEFT_NO,DEPT_NAME,LOCATION)VALUES(DEPARTMENT_SEQ.NEXTVAL,'영업부','대구');
INSERT INTO DEPARTMENT_TBL(DEFT_NO,DEPT_NAME,LOCATION)VALUES(DEPARTMENT_SEQ.NEXTVAL,'인사부','서울');
-- INSERT INTO DEPARTMENT_TBL(DEPT_NAME, LOCATION) VALUES('총무부','대구'); DEPT_NO는 기본값으로 해놔서 지금은 에러가 걸림
INSERT INTO DEPARTMENT_TBL(DEFT_NO,DEPT_NAME,LOCATION) VALUES(DEPARTMENT_SEQ.NEXTVAL,'총무부','대구');
INSERT INTO DEPARTMENT_TBL(DEFT_NO,DEPT_NAME,LOCATION) VALUES(DEPARTMENT_SEQ.NEXTVAL,'기획부','서울');
COMMIT;
*/
...
UPDATE DEPARTMENT_TBL 
    SET LOCATION = '경기' --수정할 내용(여기서 등호는 대입연산자)
 WHERE DEPT_NO = 1;       --조건문 (여기서 등호는 비교 연산자이다)
COMMIT;

--2 부서번호(DEPART가 1인 부서에 근무하는 사원들의 급여를 50만원 증가시키십시오 급여는 SALARY
...
DELETE FROM DEPARTMENT_TBL WHERE LOCATION ='대구'; -- 대구는 근무하는 사원이 없으므로 문제없이 삭제
DELETE FROM DEPARTMENT_TBL WHERE LOCATION ='서울'; -- 서울은 부서번호(DEPART)가 ONDELETE SET NULL 외래키 옵션에 의해서 NULL 로 처리된다.
COMMIT;

-- ON DELETE SET NULL 을 했기 때문에 부모가 사라져서 자동으로 NULL 처리가 됨 
